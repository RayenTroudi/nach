...
const AvailabilityPage = () => {
  ...
  const formatTime = (time: string) => {
    ...
    return `${displayHour}:${minutes} ${isPM ? t('pm') : t('am')}`;
  };
            ...
            {Object.keys(groupedSlots).length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {DAY_KEYS.map((dayKey, dayIndex) => {
                  ...
                  return (
                    <div key={dayIndex} className="bg-slate-50 dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-800">
                      <div className="flex items-center justify-between mb-3">
                        <h3 className="font-semibold text-base text-slate-950 dark:text-white">{t(dayKey)}</h3>
                        <Badge variant="outline" className="text-xs">
                          {daySlots.length} {daySlots.length === 1 ? t('slot') : t('slots')}
                        </Badge>
                      </div>

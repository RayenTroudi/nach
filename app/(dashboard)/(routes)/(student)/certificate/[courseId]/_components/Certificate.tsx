...
const Certificate = ({ user, course, certificateRef }: Props) => {
  ...
  return (
    <div
      className="h-calc(100vh-80px) flex-1  bg-white py-24 px-16 border-2 border-black dark:border-white"
      ref={certificateRef}
    >
      <div className=" bg-white  mx-auto h-full flex  flex-col gap-y-14 justify-between">
        <div className="w-full flex items-center justify-between">
          <div className="flex items-center gap-2 flex-shrink-0">
            <em className={` text-black font-bold text-4xl`}>
              We
        ...
        <div className="w-full flex flex-col gap-y-2">
          ...
          <h2 className="leading-relaxed tracking-wider text-6xl capitalize text-black font-bold">
            {course.title}
          </h2>
          ...
          <div className="flex gap-1 items-center">
            <p className="text-lg font-bold text-black"> {courseRating} </p>
            <StarIcon size={20} className="text-[#facc15]" fill="#facc15" />
            <span className="text-md text-slate-400 ml-2">
              ({ratingFrom} votes)
            </span>
          </div>
        </div>
